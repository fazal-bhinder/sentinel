...
// .wrangler/tmp/bundle-rpNK4r/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-rpNK4r/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
// src/core/canonical.ts
var PROVIDERS = ["openai", "anthropic", "cursor"];
var EVENT_TYPES = ["chat", "completion", "edit"];
var CanonicalEvent = external_exports.object({
  event_id: external_exports.string().uuid(),
  dedup_key: external_exports.string(),
  // deterministic idempotency key
  provider: external_exports.enum(PROVIDERS),
  parser_version: external_exports.number().int(),
  raw_payload_id: external_exports.string().uuid(),
  occurred_at: external_exports.string().datetime(),
  event_type: external_exports.enum(EVENT_TYPES),
  model: external_exports.string().nullable(),
  actor: external_exports.object({ raw_user_id: external_exports.string().nullable() }),
  usage: external_exports.object({
    input_tokens: external_exports.number().nullable(),
    output_tokens: external_exports.number().nullable()
  }),
  cost: external_exports.object({ amount_usd: external_exports.number().nullable() }),
  // born null, backfilled
  revision: external_exports.number().int(),
  ingested_at: external_exports.string().datetime()
});
var VOLATILE_FIELDS = [
  "event_id",
  "raw_payload_id",
...
__name(dedupKey, "dedupKey");
function deterministicUuid(seed) {
  const h = sha256(seed);
  const c = h.slice(0, 32).split("");
  c[12] = "4";
  c[16] = "89ab"[parseInt(h[16], 16) % 4];
  const x = c.join("");
  return `${x.slice(0, 8)}-${x.slice(8, 12)}-${x.slice(12, 16)}-${x.slice(16, 20)}-${x.slice(20, 32)}`;
}
__name(deterministicUuid, "deterministicUuid");

// src/core/assemble.ts
function assembleCanonical(f) {
  const dk = dedupKey({
    provider: f.provider,
    providerEventId: f.providerEventId ?? void 0,
...
__name(assembleCanonical, "assembleCanonical");
...
var openaiV1 = {
  ...
  parse(raw2) {
    ...
    return assembleCanonical({
      ...
      occurred_at: new Date((r.created ?? 0) * 1e3).toISOString(),
...
var anthropicV1 = {
  ...
  parse(raw2) {
    ...
    return assembleCanonical({
      ...
      parser_version: 1,
      providerEventId: r.id ?? null,
      occurred_at: r.created_at ?? (/* @__PURE__ */ new Date(0)).toISOString(),
      event_type: "chat",
      model: r.model ?? null,
...
var cursorV1 = {
  ...
  parse(raw2) {
    ...
    return assembleCanonical({
      ...
      occurred_at: r.timestamp ?? (/* @__PURE__ */ new Date(0)).toISOString(),
      event_type: "edit",
      model: r.model ?? null,
...
var Registry = class {
  ...
  get(provider, version2) {
    return this.map.get(`${provider}:${version2}`);
  }
  has(provider, version2) {
    return this.map.has(`${provider}:${version2}`);
  }
  versions(provider) {
    return [...this.map.values()].filter((p) => p.provider === provider).map((p) => p.version).sort((a, b) => a - b);
  }
  latest(provider) {
    const vs = this.versions(provider);
    return vs.length ? this.get(provider, vs[vs.length - 1]) : void 0;
  }
  providers() {
...
__name(semantic, "semantic");
function diffStatus(golden, candidate) {
  return stableStringify(semantic(golden)) === stableStringify(semantic(candidate)) ? "ok" : "drift";
}
__name(diffStatus, "diffStatus");
function runReplay(registry2, corpus) {
  const cells = [];
  for (const provider of registry2.providers()) {
    const items = corpus.filter((c) => c.provider === provider);
    for (const version2 of registry2.versions(provider)) {
      const parser = registry2.get(provider, version2);
      const cell = {
        provider,
        version: version2,
        total: items.length,
...
function leafPaths(value, prefix, out) {
  if (value !== null && typeof value === "object") {
    ...
    for (const k of keys) {
      leafPaths(value[k], prefix ? `${prefix}.${k}` : k, out);
    }
    return;
  }
  out.push(prefix);
...
function fingerprintPaths(raw2) {
  const out = [];
  leafPaths(raw2, "", out);
  return new Set(out);
}
__name(fingerprintPaths, "fingerprintPaths");
function fingerprint(raw2) {
  return [...fingerprintPaths(raw2)].sort().join("\n");
}
__name(fingerprint, "fingerprint");

// src/core/drift.ts
function structuralDrift(input) {
...
__name(structuralDrift, "structuralDrift");
function silentNullDrift(provider, stats, cfg = {}) {
  ...
  for (const s of stats) {
    ...
    alerts.push({
      ...
      field: s.field,
      message: `${s.field} went dark: ${(overallRate * 100).toFixed(0)}% populated historically, ${(recentRate * 100).toFixed(0)}% across the last ${s.recentSeen}. Parser never threw.`,
      details: { overallRate, recentRate, overallSeen: s.overallSeen, recentSeen: s.recentSeen }
    });
  }
  return alerts;
}
__name(silentNullDrift, "silentNullDrift");
...
function getConnectionString(env2) {
  if (env2?.HYPERDRIVE?.connectionString) {
    return env2.HYPERDRIVE.connectionString;
  }
  if (env2?.DATABASE_URL) {
    return env2.DATABASE_URL;
  }
  return process.env.DATABASE_URL ?? "postgres://oximy:oximy@localhost:5433/oximy";
}
__name(getConnectionString, "getConnectionString");
function initPool(env2) {
  if (activePool) return activePool;
  const connectionString = getConnectionString(env2);
  activePool = new esm_default.Pool({ connectionString });
  return activePool;
}
...
async function ingest(provider, raw2, registry2, opts = {}) {
  const parser = opts.version ? registry2.get(provider, opts.version) : registry2.latest(provider);
  if (!parser) throw new Error(`no parser registered for ${provider} v${opts.version ?? "?"}`);
  const golden = parser.parse(raw2);
  const fp = fingerprint(raw2);
  const cHash = contentHash(raw2);
  ...
  try {
    ...
    await client.query(
      ...
      [rawId, provider, cHash, JSON.stringify(raw2), fp, parser.version, JSON.stringify(golden)]
    );
    const upsert = await upsertEvent(client, golden, rawId, parser.version);
    const alerts = await detectDrift(client, provider, raw2, parser.expects, known);
...
async function upsertEvent(client, ev, rawId, version2) {
...
async function knownFingerprints(client, provider) {
  const res = await client.query(
...
__name(knownFingerprints, "knownFingerprints");
async function detectDrift(client, provider, raw2, expects, known) {
  const candidates = [];
  ...
  for (const a of candidates) {
    const existing = a.type === "STRUCTURAL" ? await client.query(
...
async function fieldStats(client, provider, expects) {
  ...
  return fields.map((field) => {
    ...
    const recentSeen = r.seen;
    const recentPopulated = r[col];
    return {
      field,
      overallSeen: t.seen - recentSeen,
      overallPopulated: t[col] - recentPopulated,
      recentSeen,
      recentPopulated
    };
  });
}
__name(fieldStats, "fieldStats");
...
async function vitals(window = 40) {
  const fields = [
    { key: "model", col: "model" },
  ...
  for (const [provider, rows] of byProvider) {
    ...
    const fieldVitals = fields.map((f) => {
      const series = ordered.map((r) => r[f.col] === null ? 0 : 1);
      const recSeries = recent.map((r) => r[f.col] === null ? 0 : 1);
      const rate = recSeries.length ? recSeries.reduce((a, b) => a + b, 0) / recSeries.length : 1;
...
__name(loadCorpus, "loadCorpus");

// src/api/inspect.ts
var FIELD_TO_COLUMN2 = {
  model: "model",
  "actor.raw_user_id": "raw_user_id",
  "usage.input_tokens": "input_tokens",
  "usage.output_tokens": "output_tokens",
...
__name(resolveEventId, "resolveEventId");
async function buildInspection(params, registry2) {
  const eventId = await resolveEventId(params);
  if (!eventId) return null;
  const ev = await pool.query(`SELECT * FROM events WHERE event_id = $1`, [eventId]);
  ...
  const raw2 = rp.rows[0];
  const parser = registry2.get(event.provider, event.parser_version);
  ...
  const currentPaths = raw2 ? fingerprintPaths(raw2.payload) : /* @__PURE__ */ new Set();
  const added = [...currentPaths].filter((p) => !baselinePaths.has(p)).sort();
  const removed = [...baselinePaths].filter((p) => !currentPaths.has(p)).sort();
  let silentNull = null;
  const darkField = missing.find((m) => FIELD_TO_COLUMN2[m]);
  if (darkField) {
    const col = FIELD_TO_COLUMN2[darkField];
    ...
    if (historical >= THRESHOLD && recentRate <= 1 - THRESHOLD + 1e-4) {
      silentNull = {
        field: darkField,
  ...
  const cells = runReplay(registry2, corpus).filter((c) => c.provider === event.provider);
  const regressions = cells.reduce((s, c) => s + c.throw, 0);
  const failures = detailedFailures(registry2, providerItems, event.provider);
  const replay = {
  ...
  const related = al.rows.filter(
    (a) => a.type === "SILENT_NULL" && missing.includes(a.field ?? "") || a.type === "STRUCTURAL" && (added.length > 0 || removed.length > 0)
  );
  let suggestedVersion = null;
  for (const v of registry2.versions(event.provider).filter((v2) => v2 > event.parser_version)) {
    if (!raw2) break;
    const out = registry2.get(event.provider, v).parse(raw2.payload);
  ...
  const timeline = buildTimeline(event, raw2?.received_at ?? null, {
    missing,
    structuralChanged,
  ...
  const rootCause = structuralChanged ? `Provider moved a field: ${removed.length ? removed.join(", ") : "\u2014"} \u2192 ${added.length ? added.join(", ") : "\u2014"}.` : silentNull ? `${silentNull.field} stopped populating without a parse error.` : "No incident \u2014 every expected field is populating normally.";
  const impact = missing.length ? `${affected.rows[0]?.c ?? 0} ${event.provider} events currently missing ${missing.join(", ")}.` : "No fields missing on this event.";
  const suggestedFix = suggestedVersion ? `Upgrade to ${event.provider} parser v${suggestedVersion} (reads the moved field).` : "On the latest parser \u2014 no upgrade needed.";
  const replayNote = replay.regressions === 0 ? `${replay.historical} historical ${event.provider} payloads validate successfully.` : `${replay.regressions} regression(s) across ${replay.historical} historical payloads.`;
  return {
    status,
    event: canonical,
    raw_payload: raw2?.payload ?? null,
    raw_payload_id: event.raw_payload_id,
    received_at: raw2?.received_at ?? event.ingested_at,
...
__name(buildInspection, "buildInspection");
async function fieldSeries(provider, col, window) {
  const res = await pool.query(
...
__name(fieldSeries, "fieldSeries");
function detailedFailures(registry2, items, provider) {
  const failures = [];
  for (const v of registry2.versions(provider)) {
    const parser = registry2.get(provider, v);
...
__name(detailedFailures, "detailedFailures");
function buildTimeline(e, receivedAt, flags) {
  const driftStatus = flags.silentCritical ? "fail" : flags.structuralChanged || flags.silentNull ? "warn" : "ok";
  return [
    ...
    },
    {
      ...
      status: driftStatus,
      detail: flags.silentCritical ? "SILENT_NULL critical raised" : flags.structuralChanged ? "structural shape change" : "no drift",
      at: e.ingested_at
    },
...
var registry = buildRegistry({ openaiV2: true });
...
app.post("/ingest", async (c) => {
  try {
    ...
    const result = await ingest(body.provider, body.payload, registry, {
...
app.get("/inspect", async (c) => {
  ...
  const alertId = alertIdQuery ? Number(alertIdQuery) : void 0;
...
var replayHandler = /* @__PURE__ */ __name(async (c) => {
  const corpus = await loadCorpus();
  const cells = runReplay(registry, corpus);
  return c.json({ cells, corpus_size: corpus.length, ran_at: (/* @__PURE__ */ new Date()).toISOString() });
}, "replayHandler");
